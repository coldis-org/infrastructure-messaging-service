...
# Java Opts
if [ -z "$JAVA_ARGS" ]; then
    JAVA_ARGS=" -XX:+AggressiveOpts -XX:+UseFastAccessorMethods -XX:+PrintClassHistogram -XX:+UseStringDeduplication -Dhawtio.realm=activemq -Dhawtio.offline=true -Dhawtio.role=technology-messaging-service-admin -Dhawtio.rolePrincipalClasses=org.apache.activemq.artemis.spi.core.security.jaas.RolePrincipal -Djolokia.policyLocation=${ARTEMIS_INSTANCE_ETC_URI}jolokia-access.xml"
fi
